...
While [ECE391](ECE391.md) used to be an official prerequisite and technically still is, as of recently the real (enforced) prerequisite is [ECE385](ECE385.md). Since ECE385 teaches SystemVerilog, which is used in all of the MPs for this course, it is imperative to know how to write HDL before starting this course. The only directly relevant ECE391 topics are virtual memory and I/O, and those are re-covered in ECE411 lectures anyways. However, ECE391 develops your debugging skills moreso than ECE385, which can be very useful for ECE411.
